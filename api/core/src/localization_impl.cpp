...
namespace Localization {
    ...
    /* latlon - Get the approximate latitude and longitude coordinates of the device location */
    LatLon LocalizationImpl::latlon( Firebolt::Error *err )  const
    {
        Firebolt::Error status = Firebolt::Error::NotConnected;
        LatLon latlong;
        ...
        Firebolt::Localization::JsonData_LatLon jsonResult;
        status = FireboltSDK::Gateway::Instance().Request("localization.latlon", jsonParameters, jsonResult);
        if (status == Firebolt::Error::None) {
            FIREBOLT_LOG_INFO(FireboltSDK::Logger::Category::OpenRPC, FireboltSDK::Logger::Module<FireboltSDK::Accessor>(), "Localization.latlon is successfully invoked");
            ASSERT((jsonResult.Length() == 0) || (jsonResult.Length() == 2));
                if (jsonResult.Length() == 2) {
                    latlong.first = jsonResult.Get(0);
                    latlong.second = jsonResult.Get(1);
    	    }
